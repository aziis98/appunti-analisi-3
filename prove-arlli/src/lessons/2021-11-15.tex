...
Poniamo $c_n(t) \coloneqq c_n(u(t, \curry))$. Sappiamo che per $t > 0$ vale $\dot c_n(t) \stackrel{(\ast)}{=} c_n(u_t(t, \curry)) \stackrel{(\ast \ast)}{=} c_n(u_{xx}(t, \curry)) = -n^2 c_n(t)$, dove $(\ast)$ segue dal Lemma 8 e $(\ast \ast)$ segue dalla regolarità dei coefficienti. Dunque i coefficienti $c_n$ risolvono il problema di Cauchy
$$
\begin{cases}
	\dot y = -n^2 y \\
	y(0) = c_n^0
