...
\item La serie di Fourier converge totalmente\footnote{Ricordiamo che $\sum a_n(x)$ converge totalmente se $\sum \norm{a_n (x)}_\infty < +\infty$.}.
\end{enumerate}
...
\underbrace{\leq}_{\text{C.S.}} \overbrace{\left( \sum_{n \in \Z} \left| n \right|^2 \left| c_n(f) \right|^2  \right)^{1/2}}^{< +\infty \text{ per il punto ii)}} \cdot \left( \sum_{n \in \Z} \frac{1}{\left| n \right|^{2-2\alpha }}  \right)^{1/2}. 
$

\item Dal punto precedente con $\alpha = 0$ otteniamo $\ds \sum_n \norm{c_n(f) e^{inx}}_\infty = \sum_{n \in \Z} |n|^0 \left| c_n(f) \right| < +\infty $.
\end{enumerate}
...
\textbf{Dimostrazione.} 
Sia $h \leq k - 1$ allora osserviamo che
$$
\begin{gathered}
	D^h(c_n(f) e^{inx}) = (in)^h c_n(f) e^{inx} \\
	\implies \norm{D^h(c_n(f) e^{inx})}_\infty = \norm{(in)^h c_n(f) e^{inx}}_\infty \leq |n|^h |c_n| \leq |n|^{k-1} |c_n| \\
	\implies \sum_n \norm{D^h(c_n(f) e^{inx})}_\infty \leq \sum_n |n|^{k-1} |c_n| < +\infty 
\end{gathered}
$$
dunque la serie $\ds\sum D^h(c_n(f) e^{inx})$ converge totalmente. Per un teorema di Analisi 2 abbiamo che quindi $\exists \tilde f \in C^{k-1}$ tale che $\ds\sum c_n(f) e^{inx} \to \tilde f$ totalmente (in particolare $\ds\sum D^h (c_n(f) e^{inx}) \to D^h \tilde f$) da cui segue che converge uniformemente e quindi anche in $L^2$. Dunque $f = \tilde f$ in $L^2 \implies f = \tilde f$ q.o. e poiché sono entrambe continue $f = \tilde f$. \qed 
% Preso $h=0,1,\ldots,k-1$ vale
% \begin{align*}
% D^h (c_n(f) e^{inx}) & = c_n(f) (in)^h e^{inx} \\
% \norm{D^h (c_n(f) e^{inx})}_\infty & = \left| c_n(f) \right| \left| n \right|^h \leq \left| c_n(f) \right| \left| n \right|^{k-1}.
% \end{align*}
% % TODO(aziis98): e quindi esiste \tilde f tale che quella roba converge a D^h \tilde f...
...
In particolare abbiamo usato il seguente lemma.
...
Ricordiamo che non è esattamente la convoluzione di $f$ e $D_N$ in quanto il dominio è solo $[-\pi, \pi]$ e non tutto $\R$. In particolare in $(\star)$ abbiamo usato il seguente lemma.
