...
\subsection{Disuguaglianza di Young per convoluzione}
...
\begin{itemize}
	...
	\item
		...
		\begin{aligned}
			...
			&= \int f_2(y) \int f_1(y - x) h^{p-1}(x) \dd x \dd y 
...
\begin{enumerate}
	...
	\item \label{item:20ott_th4_4}
		Se $1 < p_1, p_2 < +\infty$ allora $f_1 \ast f_2 \to 0$ per $|x| \to +\infty$, ovvero è infinitesima a $\pm\infty$.
\end{enumerate}

Premettiamo i seguenti risultati.

\textbf{Proposizione 5.}
Data $f \in L^p(\R^d)$ con $p < +\infty$ la mappa di traslazione
$$
\begin{array}{cccc}
