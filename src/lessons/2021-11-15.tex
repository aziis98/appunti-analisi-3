...
$$
con $c_n \coloneqq c_n(u(t, \curry))$ da cui derivando formalmente dentro le sommatorie otteniamo che $u_x$ e $u_{xx}$ sono
$$
\begin{gathered}
	\sum_{n \in \Z} \dot c_n(t) e^{inx} = u_t = u_{xx} = \sum_{n \in \Z} -n^2 c_n(t) e^{inx} \\
	u_t = u_{xx} \Leftrightarrow \forall n \, \forall t \;\; \dot c_n(t) = -n^2 c_n(t)
	\quad
	\text{e} 
	\quad
	u(0, \curry) = u_0 \Leftrightarrow c_n(0) = c_n^0 \coloneqq c_n(u_0)
\end{gathered}
$$
...
Se $u_0$ (presa in $L^2$) è tale che $\ds \sum_{n \in \Z} |c_n| < +\infty$ (ad esempio se $u_0 \in C^1$ ed è $2\pi$-periodica) allora
$$
u(t, x) = \sum_{n \in \Z} c_n^0 e^{-n^2 t} e^{inx}
$$
...
$$
allora $u \coloneqq \sum_n u_n$ è una funzione ben definita su $R$ e $C^k$ e $D^{\underline u} \, u = \sum_n D^{\underline h} \, u_n$ per ogni $\underline h$ con $|\underline h| \leq k$.
...
\begin{enumerate}
	\item $u_0$ reale $\implies c_{-n}^0 = \overline{c_n^0} \implies c_{-n}^0 e^{-(-n)^2 t} = \overline{c_n^0 e^{-n^2 t}} \iff c_{-n}(u(t, \curry))$.

	\item Sia $R \coloneqq [0, +\infty) \times \R$, $\norm{u_n}_{L^\infty(R)} = |c_n^0|$ dunque
		$\ds\sum_{n \in \Z} u_n$ conv. totalmente su $R$ e quindi $u$ è ben definita e \textit{continua} su $R$.
	...
	\item $u$ è $2\pi$-periodica in $x \implies$ valgono le condizioni al bordo $u(0, \curry) =$ serie di Fourier di $u_0 = u_0$. Inoltre $(u_n)_t = (u_n)_{xx} \implies \sum (u_n)_t = \sum (u_n)_{xx} \implies u_t = u_{xx}$ per $t \gneq 0$.
\end{enumerate}
\qed
...
Sia $u \colon [0, T) \times [-\pi, \pi] \to \C$ continua, $C^1$ nel tempo e $C^2$ nello spazio per $t > 0$. Se $u$ risolve \ref{eq:15nov2021_problem_1} su $t > 0$ allora $u$ è unica.
