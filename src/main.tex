...
Basta dimostrare che $\ds \mu \left( \bigcup_i E_i \right) \leq \sum_{i}^{} \mu(E_i)$. Infatti per quanto visto prima $\ds \mu(E) \leq \mu \left( \bigcup_i E_i \right)$.

Si dimostra per induzione la seguente
\[
\mu \left( \bigcup_{i = 1}^N E_i \right) \leq \sum_{i=1}^{N} \mu(E_i).
\] 
...
inoltre
\[
	\mu \left( \left\{ x \in X \mymid f(x) > 0 \right\} \right) = \lim_{n \to +\infty} \left\{ x \in X \mymid f(x) \geq \frac{1}{n} \right\},
\] 
dove
\[
	\mu \left( \left\{ x \in X \mymid f(x) > 0 \right\} \right) >0 \longiff \exists \bar{n} \mid \mu \left( \left\{ x \in X \mymid f(x) \geq 1 / \bar{n} \right\} \right) > 0.
\] 

Allora
\[
	\int_X f \dd \mu \geq \int\limits_{\mathclap{\left\{ x \mymid f(x) \geq \frac{1}{n} \right\}}}^{} f \dd \mu \geq
	\frac{1}{n} \mu \left(  \left\{ x \mymid f(x) \geq \frac{1}{n} \right\} \right). 
\] 
...
Se $\norm{ f }_{\infty} < + \infty$, allora dico che esiste una costante $L > 0$ con $L = \norm{ f }_{\infty, X}$, tale che 
\[
...
\] 
Definisco $\ds N = \bigcup_{m} N_m$, da cui si ottiene
\[
...
\begin{cases}
	...
	\frac{\rho^{n-\alpha}}{n - \alpha} \quad \text{altrimenti} 
\end{cases} 
\] 
...
\[
	\norm{ f \cdot g }_r \leq \norm{ f }_{p_1} + \norm{ g }_{p_2}.
\] 
\textit{Suggerimento.} Usare Holder osservando che $1 = \frac{r}{p_1} + \frac{r}{p_2} = \frac{1}{\left( p_1/r \right)} + \frac{1}{\left( p_2/r \right)}$.
