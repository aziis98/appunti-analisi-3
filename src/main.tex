...
Vediamo ora alcune classi di funzioni dense in $L^p$ che risulteranno utili come strumento per fare le dimostrazioni.

Per ora consideriamo $(X, \mathcal A, \mu)$ in generale.

\textbf{Esercizio.} 
Le funzioni limitate ed in $L^p$ sono dense in $L^p$.

\textbf{Dimostrazione.}
Data $f \in L^p(X)$ cerchiamo $f_n \in L^p(X) \cap \text{Limitate}$ tali che $f_n \to f$ in $L^p$, consideriamo
$$
...
$$
per la \textit{convergenza puntuale} basta osservare che per $n \geq |f(x)|$ abbiamo che $\forall x \; f_n(x) = f(x) \implies f_n(x) \xrightarrow{n} f(x) \implies |f_n(x) - f(x)|^p \to 0$.

Infine basta usare convergenza dominata usando come dominazione direttamente $|f(x) - f_n(x)| \leq |f(x)| \implies |f(x) - f_n(x)|^p \leq |f(x)|^p$ e notiamo che $|f|^p \in L^1(X)$.
\qed

\textbf{Proposizione.}
Sia $\tilde{\mathscr S} \coloneqq \operatorname{Span}(\{ \One_E \mid E \in \mathcal A, \mu(E) < +\infty \})$, allora $\tilde{\mathscr S}$ è denso in $L^p(X)$.

\textbf{Dimostrazione.}
Data $f \in L^p(X)$ cerchiamo una successione che approssima $f$ in $\tilde{\mathscr S}$.
\begin{itemize}
	\item 
		\textit{Caso 1}: Se $f \geq 0$ allora fissiamo $\epsilon > 0$ e per ogni $k = 1, 2, \dots$ poniamo
		$$
		A_k^\epsilon := \{ x \mid k \epsilon \leq f(x) \leq (k+1) \epsilon \}
		$$
		e risulta che $A_k^\epsilon$ è misurabile ed ha misura finita, inoltre poniamo
		$$
		f_\epsilon(x) := \sum_{1 \leq k \leq 1 / \epsilon^2} k \epsilon \cdot \One_{A_\epsilon^k}(x) \in \tilde{\mathscr S}
		$$
		[TODO: Disegnino]

		Osserviamo che vale anche $f_\epsilon(x) = \max\{ k \epsilon \mid k \epsilon \leq f(x) \text{ e } k \leq 1 / \epsilon^2 \}$ e mostriamo la seguente\footnote{Notiamo che qui stiamo usando il teorema di convergenza dominata non ad una successione di funzioni ma ad una famiglia parametrizzata da $\epsilon$, però si può verificare facilmente che il teorema si può estendere in questo caso semplicemente prendendo $\epsilon = 1 / n$.}
		$$
		...
		\begin{itemize}
			...
			\item \textit{Dominazione}: Possiamo usare $|f(x) - f_\epsilon(x)|^p \leq |f(x)|^p < +\infty$ in quanto $f \in L^p(X)$.
		\end{itemize}
	...
	\item 
		\textit{Caso 2}:
		Sia $f \colon X \to \R$ allora rifare la dimostrazione precedente oppure semplicemente consideare $f_\epsilon \coloneqq (f^+)_\epsilon - (f^-)_\epsilon$.
...
Intuitivamente $\tilde{\mathscr S}_\ell$ è denso in $\tilde{\mathscr S}$ che a sua volta è denso in $L^p$ (topologicamente è quasi ovvio mentre usando la definizione di densità per successioni bisogna pasare per un ragionamento diagonale).
...
\textbf{Dimostrazione.} ($C_C(\R)$ è denso in $L^p(X)$)
