...
\textbf{Proposizione.}
Sia\footnote{Lo span è inteso come combinazioni lineari} $\tilde{\mathscr S} \coloneqq \operatorname{Span}(\{ \One_E \mid E \in \mc A, \mu(E) < +\infty \})$, allora $\tilde{\mathscr S}$ è denso in $L^p(X)$.
...
\textbf{Esercizio 1.}
Sia $H = L^2(-1, 1)$ e sia $V = \operatorname{Span}\{ 1, x, x^2 \}$, verificare che sia un sottospazio chiudo e calcolare la proiezione di $\sin x$ su $V$.
...
\begin{enumerate}
	...
	\item $\mathcal F$ è completo (richiede Stone-Weierstrass)
		...
		In particolare noi vorremmo usare questo teorema applicandolo alle funzioni $2\pi$-periodiche ristrette a $[-\pi, \pi]$, però queste non verificano la separazione dei punti in quanto per la periodicità $f(-\pi) = f(\pi)$.

		Vediamo però nel seguente corollario che possiamo estendere leggermente il teorema passando dai quozienti topologici.

		\textbf{Corollario.}
		Sia $\mathcal A$ una sottoalgebra di $C(K)$ (o analogamente per $C(K; \C)$) che contiene le costanti (e nel caso complesso chiusa per coniugio). Definiamo la relazione $x_1 \sim x_2$ se $f(x_1) = f(x_2)$ per ogni $f \in \mathcal A$ allora 
		$$
		\overline{\mathcal A} = \{ f \in C(K) \mid x_1 \sim x_2 \Rightarrow f(x_1) = f(x_2) \} \eqqcolon X
		$$
		...
		Si applica Stone-Weierstrass a $\sfrac{K}{\sim}$, è chiaro che $\overline{\mathcal A} \subset X$, vediamo che $X \subset \overline{\mathcal A}$. 
		...
		Sia ora $K = [-\pi, \pi]$ e consideriamo
		$$
		\mathcal A = \operatorname{Span}_\C(\mathcal F) = \left\{ \sum_{n \in \Z} c_n e^{inx} \right\} = \{ p(e^{inx}) \mid p \text{ polinomio a esponenti interi} \}
		$$
		segue che $\mathcal A$ è una sottoalgebra, separa i punti di $K$ tranne $-\pi$ e $\pi$ ed è chiuso per coniugio.

		Per il corollario $\overline{\mathcal A}^{\,C} = \{ f \in C([-\pi, \pi]; \C) \mid f(-\pi) = f(\pi) \}$. Se invece facciamo la chiusura rispetto ad $L^2$ abbiamo che $\overline{\mathcal A}^{\,L^2} \supseteq \{ f \in C([-\pi, \pi]; \C) \mid f(-\pi) = f(\pi) \}$ per il fatto che convergenza uniforme $\implies$ convergenza in $L^2$ per spazi di misura finita.

		Poi $\overline{\mathcal A}^{\,L^2} \supseteq \{ f \in C([-\pi, \pi]; \C) \}$, data $f \in C([-\pi, \pi]; \C)$ la approssimiamo in $L^2$ con $f_n = f \cdot \varphi_n$, dove le $\varphi_n$ sono tali che $\varphi_n(-\pi) = \varphi_n(\pi) = 0$, $\varphi_n = 1$ su $[1/n - \pi, \pi - 1/n]$ e interpolata linearmente nell'intervallo rimanete.
