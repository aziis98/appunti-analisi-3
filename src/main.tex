...
\subsection{Esempi di basi Hilbertiane}
...
\subsubsection{Base di Haar}

[TODO: Disegnino super wow]

Vediamo la base di Haar data da due indici $n, k$ dove $n$ indica l'ampiezza delle ``onde'' (wavelet) e $k$ il posizionamento dell'onda. Sia $n \in \N$ e $k = 0, \dots, 2^n$ e poniamo
$$
g^{0,0} \coloneqq 1
\qquad
...
$$
Inoltre $\| g^{n, k} \|_{L^2[0, 1]} = 1$ ed anche $\| g^{0,0} \|_{L^2[0, 1]} = 1$. Vedremo che $\{ g^n,k \}_{n,k} \cup \{ g^{0,0} \}$ formano un sistema ortonormale.
...
$$
\One_{\left[ \frac{k-1}{2^n}, \frac{k}{2^n} \right]}
$$
