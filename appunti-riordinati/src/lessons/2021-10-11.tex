...
$$
	f_n(x) \coloneqq (f(x) \land n) \lor (-n)
$$
...
Sia $\wtilde{\mathscr S} \coloneqq \spn(\{ \One_E \mid E \in \mc A, \mu(E) < +\infty \})$, allora $\wtilde{\mathscr S}$ è denso in $L^p(X)$.
}
...
\mybox{%
\textbf{Proposizione 2bis} (Generalizzata). Sia $\ds \wtilde{\mathscr S} \coloneqq \left\{ \sum_i \alpha_i \One_{E_i} \mid \alpha_i \in \R^d, E_i \in \mc A, \mu(E_i) < +\infty \right\}$. Allora $\wtilde{\mathscr S}$ è denso in $L^p(X; \R^d)$.
}
...
Sia $\wtilde{\mathscr S}_\ell \coloneqq \{ \sum_i \alpha_i \One_{E_i} \mid \alpha_i \in \R^d, E_i \in \mc A, \mu(E_i) < +\infty, E_i \text{ limitati} \}$ allora $\wtilde{\mathscr S}_\ell$ è denso in $L^p(X; \R^d)$ per $p < +\infty$.
}
...
% @ar-lli (?) (usando la definizione di densità topologica la tesi è quasi ovvia mentre usando la definizione per successioni bisogna passare per un procedimento diagonale).
...
%
\begin{align*}
	\norm{f_\epsilon - \One_E}_p^p & = \int_{A_\epsilon \setminus C_\epsilon} |f_\epsilon - \One_E|^p \dd x \leq \int_{A_\epsilon \setminus C_\epsilon } \dd x = \left| A_\epsilon \setminus C_\epsilon  \right| \leq \epsilon \\
...
% \mybox{%
% \textbf{Nota.} È denso anche l'insieme delle funzioni continue a supporto compatto ristretto a $X$ e si indica con $\mc{C}_C^0(\R^n)$.
% }
