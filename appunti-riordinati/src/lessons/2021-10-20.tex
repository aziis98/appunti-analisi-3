...
\textbf{Proposizione 1.} Date $f_1,f_2 \colon \R^d \to \R$ e $x \in \R^d$, se $|f_1| \ast |f_2| (x) < +\infty$ allora $f_1 \ast f_2(x)$ è ben definito, reale e in particolare vale 
$$
	\left| f_1 \ast f_2(x) \right| \leq |f_1| \ast |f_2|(x).
$$

\textbf{Dimostrazione.} 
% Basta osservare che,
% $$
% 	\leq \int_{\R^d} |f_1(x-y) \cdot f_2(y)| \dd y = |f_1| \ast |f_2|(x) < +\infty.
% $$
% \qed
Se 
$$
	|f_1| \ast |f_2|(x) = \int_{\R^d} \left| f_1(x-y) f_2(y) \right| \dd y
$$
è finito allora $f_1(x-\cdot) f_2(\cdot) \in L^1(\R^d)$ per ogni $x$ e quindi
$$
	f_1 \ast f_2(x) \coloneqq \int_{\R^d} f_1(x-y) f_2(y) \dd y
$$
è ben definito e
$$
	|f_1 \ast f_2(x)| \leq \int_{\R^d} \left| f_1(x-y)f_2(y) \right| \dd y = |f_1| \ast |f_2|(x)
$$
\qed
...
$$
	...
	\Longrightarrow |f_1| \ast |f_2|(x) < +\infty \quad \wtilde{\forall} x \in \R^d.
$$
e per la Proposizione~1 segue la buona definizione $\wtilde{\forall} x \in \R^d$.
