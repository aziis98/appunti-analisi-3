...
\textbf{Notazione.} $R \coloneqq $ rettangolo di $\R^d$.
...
In particolare se $\sum_n \norm{D^{\underline{h}}u_n}_\infty < +\infty$ per ogni $\underline{h} \in \N^d$ allora $u$ è $C^\infty$.
}

\mybox{%
\textbf{Lemma. 6}
Data $u \colon R \to \C$ e rettangoli $R_i \subset R$ relativamente aperti in $R$ tali che $u$ è $C^k$ sugli $R_i$ per ogni $i$ allora $u$ è di classe $C^k$ su $\wtilde R \coloneqq \bigcup_i R_i$.
}
...
\begin{itemize}
	...
	\item Se $u$ è continua $c_n$ è continua su $I$.

	\item Se $u$ è $C^k$ allora $c_n$ è $C^k$ su $I$ e $c_n(D_t^h u(t, \curry)) = D_t^h c_n(u(t, \curry))$ per $h \leq k$.
