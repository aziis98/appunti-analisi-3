...
% \hypertarget{teo:della-base-hilbert}
\mybox{%
...
\begin{enumerate}
	...
	% Se $x - \overline{x} \perp e_n \mquad \forall n$, allora $x - \overline{x} \perp \overline{\spn(\mc{F})}$, ovvero $\overline{x}$ è la proiezione di $x$ su $\overline{\spn(\mc{F})}$.
	\item \label{item:27ott_thm1_4}
	Vale $x - \overline{x} \perp e_n \mquad \forall n$ e in particolare $x - \overline{x} \perp \overline{\spn(\mc{F})}$, ovvero $\overline{x}$ è la proiezione di $x$ su $\overline{\spn(\mc{F})}$.
...
% Sia $X = \left\{ \mc{F} \; \text{sistema ortonormale di } H \text{ tale che } \wtilde{\mc{F}} \subset \mc{F} \right\}$.
% Per Zorn, $X$ contiene un elemento massimale. Denotiamolo con $\wtilde{\mc{F}}$. Allora $\wtilde{\mc{F}}$ è una base di Hilbert.
...
\begin{enumerate}
	\item $H = V + V^\perp$, cioè per ogni $x \in H$ esiste $\overline{x} \in V$ e $\wtilde{x} \in V^\perp$ tale che $x = \overline{x} + \wtilde{x}$.

	\item Gli elementi $\overline{x}$ e $\wtilde{x}$ sono univocamente determinati (e indicati con $x_V$ e $x_V^\perp$).
...
% Tali intervalli si possono ottenere normalizzando $g^{n,k} + 2^\frac{n-1}{2} g^{0, 0}$ otteniamo uno degli intervalli di sopra di lunghezza $1 / 2^{n+1}$.
