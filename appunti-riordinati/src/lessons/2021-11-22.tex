...
\mybox{%
\textbf{Teorema 2.}
Siano $u_0, u_1 \colon [-\pi,\pi] \to \C$ continue e tali che 
$$
	\sum n^2 |c_n^0| < +\infty \qquad \qquad \sum |n| \cdot |c_n^1| < +\infty.
$$
Allora ($*$) definisce una funzione $u \colon \R \times \R \to \C$ di classe $C^2$, $2\pi$-periodica in $x$ che risolve (P). Inoltre, se $u_0,u_1$ sono reali $u$ è reale.
}
