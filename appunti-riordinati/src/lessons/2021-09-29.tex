...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \textit{Composizione con funzione continua}: Se $f \colon X \to Y$ misurabile e $g \colon  Y \to Y'$ continua, allora $g \circ f$ è misurabile.
...
\textbf{Definizione.}
Definiamo la classe delle \textbf{funzioni semplici} come
$$
...
Date $f_n \colon  X \to [0,+\infty]$ misurabili, tali che $f_n \uparrow f$ ovunque in $X$, allora\footnote{\textit{Mnemonica:} $\ds \sup_n \int_X f_n \dd \mu = \int_X \sup_n f_n \dd \mu$}
$$
\lim_{n \to +\infty} \int_{X} f_n \dd \mu = \int_X f \dd \mu
$$
ed in particolare il termine a sinistra è crescente quindi è proprio un $\sup$ ovvero
$\lim_{n \to +\infty} \int_{X} f_n \dd \mu = \sup_n \int_X f_n \dd \mu.$
...
Date $f_n \colon  X \to \R$ (o anche $\R^n$) misurabili con le seguenti proprietà
\begin{itemize}
...
\textbf{Osservazioni.}
\begin{itemize}
	\item $\R^n$ e $\mathscr L^n$ possono essere sostituiti da $X$ e $\wtilde{\mu}$.
...
\textbf{Formula di cambio di variabile applicata a funzioni radiali.}
